...
## 실행 방법
1) 프로젝트 이동
- `cd /home/ubuntu/project/lms`

2) 환경값 준비
- 민감정보(DB/메일 계정)는 코드에 하드코딩하지 않고 환경변수로 주입합니다.
- 기본 키: `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`, `MAIL_HOST`, `MAIL_PORT`, `MAIL_USERNAME`, `MAIL_PASSWORD`, `MAIL_FROM`
- 기본 설정 파일: `src/main/resources/application.yaml`
- 로컬/운영 프로필 파일: `application-local.yaml`, `application-prod.yaml`

3) DB 준비
- MySQL에 `lms_db` 스키마 및 사용자 생성
- 마이그레이션 스크립트 실행 순서:
  - `db/migrations/V20260227_01_core_lms_schema.sql`
  - `db/migrations/V20260312_01_courses_selected_days.sql`
  - `db/migrations/V20260312_02_job_groups_levels.sql`
  - `db/migrations/V20260312_03_support_posts_writer_keys.sql`
  - 그 외 `V2026...` 파일을 번호 순서대로 실행
